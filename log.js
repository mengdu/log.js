...
const debug = process.argv.indexOf('--dev') > -1 ? true : false
const showPath = process.argv.indexOf('--dev-show-path') > -1 ? true : false
...
const getLocation = function (callsite) {
  if (!showPath) {
    var fileName = callsite.getFileName().replace(rootName, '')
    return '\x1B[90m['+ fileName +':'+ callsite.getLineNumber() +']\x1B[39m'
  }
  return '\x1B[90m['+ callsite.getFileName() +':'+ callsite.getLineNumber() +']\x1B[39m'
}
